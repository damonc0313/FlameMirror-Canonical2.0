...
class CodeGenerator:
    ...
    def generate(
        ...
        *,
        ...
        use_ml = self.ml_enabled and self.ml_model is not None
        if "tests" in {p.lower() for p in plan_step.target_file.parts}:
            use_ml = False

        if use_ml:
            if self.ml_model is None:  # pragma: no cover - defensive guard
                raise RuntimeError("ML generation requested but no model is configured")
        ...
        if template_key and template_key in self.templates:
            ...
            if "tests" in {p.lower() for p in plan_step.target_file.parts}:
                body = (
                    ...
                    "    \"\"\"Placeholder test generated by FlameMirror.\"\"\"\n"
                    "    assert True\n"
                )
            else:
                body = (
